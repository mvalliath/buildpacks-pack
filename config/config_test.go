	...
	when("#New", func() {
		when("no config on disk", func() {
			it("writes the defaults to disk", func() {
				...
				h.AssertNil(t, err)
				h.AssertContains(t, string(b), `default-builder-image = ""`)
				h.AssertEq(t, subject.DefaultBuilderImage, "")
			})
...
`), 0666))
			...
			it("loads the saved value", func() {
				...
				h.AssertEq(t, subject.DefaultBuilderImage, "some/builder")
			})
		})
	})
...
`), 0666))
		...
		it("sets the default-builder", func() {
			...
			h.AssertEq(t, subject.DefaultBuilderImage, "new/builder")
		})
	})
