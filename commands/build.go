...
func Build(logger *logging.Logger, fetcher pack.Fetcher) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: logError(logger, func(cmd *cobra.Command, args []string) error {
			...
			if bf.Config.DefaultBuilderImage == "" {
...
func suggestBuilders(logger *logging.Logger) {
	...
	for name, image := range suggestedBuilders {
		tw.Write([]byte(fmt.Sprintf("\t%s:\t%s\t\n", name, image)))
	}
	tw.Flush()
	logger.Info("\nSample builders:")
...
func buildCommandFlags(cmd *cobra.Command, buildFlags *pack.BuildFlags) {
	...
	cmd.Flags().StringSliceVar(&buildFlags.Buildpacks, "buildpack", nil, "Buildpack ID or the path to a Buildpack directory"+multiValueHelp("buildpack"))
}
